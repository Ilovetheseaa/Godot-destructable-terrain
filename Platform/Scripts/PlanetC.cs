...
public partial class PlanetC : Node
{
	...
	public void clip(Vector2[] poly, Vector2 pos)
	{

		// offsets the polygon
		var offset_poly = new Transform2D(0, pos) * poly;

		foreach (CollisionPolygon2D affected_poly in static_body.GetChildren()){
			...
			if ( des.Count() == 0){
				...
				if(des[0] != affected_poly.Polygon){
					affected_poly.CallDeferred("update_pol", des[0]);
				}
