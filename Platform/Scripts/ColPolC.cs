...
{
	Vector2 avg_position = Vector2.Zero;
	Polygon2D mesh_pol;
	public void set_avg_position(Vector2 pos){
		avg_position = pos;
	}
	public override void _Ready()
	{
		...
		mesh_pol = GetNode<Polygon2D>("Polygon2D");
		mesh_pol.Polygon = Polygon;
	}
	...
	public void update_pol(Vector2[] polygon_points){
		...
		if (double_polygon_points.Length < 3){
			...
			mesh_pol.Polygon = Geometry2D.ConvexHull(double_polygon_points);
		}
		else{
			// if everything is fine set the polygon
			SetDeferred("polygon", double_polygon_points);
			mesh_pol.Polygon = double_polygon_points;
		}
